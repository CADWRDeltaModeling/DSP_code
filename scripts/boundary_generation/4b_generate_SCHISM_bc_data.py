...
def get_pathname(
    dss_filename, b_part, c_part, e_part=None, f_part=None, filter_b_part_numeric=None
):
...
def make_links(start_date, end_date, src_dir, src_dir_narr, link_dir):
    ...
    while current <= end_date:
        # Air data
        # Ours
        src_str_air = os.path.join(
            src_dir,
            "baydelta_schism_air_%s%02d%02d.nc"
            % (current.year, current.month, current.day),
        )
        src_str_air = os.path.relpath(src_str_air, link_dir)
        # NARR
        src_str_rad = os.path.join(
            src_dir_narr,
            "%4d_%02d/narr_rad.%4d_%02d_%02d.nc"
            % (current.year, current.month, current.year, current.month, current.day),
        )
        src_str_rad = os.path.relpath(src_str_rad, link_dir)
        src_str_prc = os.path.join(
            src_dir_narr,
            "%4d_%02d/narr_prc.%4d_%02d_%02d.nc"
            % (current.year, current.month, current.year, current.month, current.day),
        )
...
class ModelBCGen(object):
    ...
    def __init__(self, yml_fname, model_type, machine, write_meta_bash=False):
        ...
        for case in self.case_items:
            ...
            if cname not in self.cases_exclude:
                ...
                crange = [case.get("case_start"), case.get("case_end")]
        ...
        if model_type.lower() == "schism" and write_meta_bash:
            ...
            for bash_cat in self.bash_file_dict:
                ...
                with open(
                    os.path.join(self.model_dir, f"{bash_cat}_meta_bash.sh"), "w"
                ) as mbf:
                    mbf.write(meta_bash)
    ...
    def setup_schism_case(self, cname, case_data_dir, crange, cperts, case_year):
        ...
        if not any("tide" in cp for cp in cperts):
            shutil.copyfile(
                self.noaa_pt_reyes, os.path.join(modcase_dir, "pt_reyes.csv")
            )
            shutil.copyfile(
                self.noaa_monterey, os.path.join(modcase_dir, "monterey.csv")
            )
            tidal_pert = ""
        ...
        for cp in cperts:
            ...
            print(f"\t - {cp}")
            if (
                "components" in pdict.keys()
            ):  # perturbation is comprised of multiple inputs
                ...
                for comp in cdict:
                    ...
                    if "flux_col" in model_setup.keys():
                        ...
                        perturbed_th, fluxes_out = self.format_schism_th(
                            subout_dir,
                            comp["model_input"],
                            modcase_dir,
                            crange,
                            ...
                            fcol=model_setup["flux_col"],
                            enc_cp=cp,
                            sign_change=sign_change,
                        )
                    else:
                        if "tide" in cp:
                            perturbed_th, tidal_pert = self.format_schism_th(
                                subout_dir,
                                comp["model_input"],
                                modcase_dir,
                                crange,
                            ...
                            perturbed_th = self.format_schism_th(
                                subout_dir,
                                comp["model_input"],
                                modcase_dir,
                                crange,
        ...
        if write_flux:
            ...
            clip_file_to_start(
                ...
                outpath=clip_fn,
                start=dt.datetime(
                    year=crange[0].year, month=crange[0].month, day=crange[0].day
                ),
            )
        ...
        fmt_string_file(
            self.param_tropic_base,
            os.path.join(modcase_dir, "param.nml.tropic"),
            run_time_dict,
            method="replace",
        )
        ...
        fmt_string_file(
            self.param_clinic_base,
            os.path.join(modcase_dir, "param.nml.clinic"),
            run_time_dict,
            method="replace",
        )
        ...
        fmt_string_file(
            self.int_vars,
            os.path.join(modcase_dir, "interpolate_variables.in"),
            run_time_dict,
            method="replace",
        )
        ...
        for th in perturbed_th.keys():
            ...
            linked_th_file_strings += (
                f"ln -sf {os.path.basename(perturbed_th[th])} {th}\n"
            )
        ...
        for fn in list(set(self.linked_ss_th_files) - set(perturbed_th.keys())):
            ...
            clip_fn = os.path.join(modcase_dir, fn)
            clip_file_to_start(
                f'{self.dcd_repo}/{fn.replace(".th","_dated.th")}',
                outpath=clip_fn,
                start=dt.datetime(
                    year=crange[0].year, month=crange[0].month, day=crange[0].day
                ),
        ...
        bash_case_tropic = os.path.join(
            modcase_dir, os.path.basename(self.bash_tropic).replace("CASENAME", cname)
        )
        slurm_case_tropic = [
            os.path.join(modcase_dir, os.path.basename(st).replace("CASENAME", cname))
            for st in self.slurm_tropic
        ]

        # CLINIC BASH
        bash_case_clinic = os.path.join(
            modcase_dir, os.path.basename(self.bash_clinic).replace("CASENAME", cname)
        )
        slurm_case_clinic = [
            os.path.join(modcase_dir, os.path.basename(sc).replace("CASENAME", cname))
            for sc in self.slurm_clinic
        ]
        ...
        for mesh_info in self.meshes:
            ...
            if (
                meshcase not in self.meshcases_exclude
                and meshname not in self.meshes_exclude
            ):
                ...
                meshcase_dir = os.path.join(self.simulations_dir, f"{meshname}_{cname}")
                # This is where simulations will be run
                ...
                if self.machine.lower() == "hpc5":
                    make_links(
                        crange[0],
                        crange[1],
                        self.env_vars["sflux_src_dir"],
                        self.env_vars["sflux_narr_dir"],
                        sflux_dir,
                    )
                elif self.machine.lower() == "azure":
                    mk_links_file = self.inputs.get("make_links").format_map(
                        self.env_vars
                    )
                    fmt_string_file(
                        mk_links_file,
                        ...
                        ),
                        {
                            **{
                                ...
                                "{src_dir}": os.path.relpath(
                                    self.env_vars["sflux_src_dir"],
                                    os.path.join(meshcase_dir, "sflux"),
                                ),
                                "{src_dir_narr}": os.path.relpath(
                                    self.env_vars["sflux_narr_dir"],
                                    os.path.join(meshcase_dir, "sflux"),
                                ),
                                "{link_dir}": "./",
                ...
                shutil.copyfile(
                    os.path.join(self.env_vars["exp_dir"], "sflux_inputs.txt"),
                    os.path.join(meshcase_dir, "sflux/sflux_inputs.txt"),
                )
                ...
                for gf in self.geometry_files:
                    infile = os.path.join(
                        mesh_input_dir.format(**self.env_vars),
                    ...
                    if not os.path.exists(infile):
                        ...
                        shutil.copyfile(infile, os.path.join(meshcase_dir, inbase))

                # copy th files to this dir
                print("\t copy time history files")
                for th in th_files:
                    shutil.copyfile(
                        th, os.path.join(meshcase_dir, os.path.basename(th))
                    )
                ...
                shutil.copyfile(
                    os.path.join(modcase_dir, "param.nml.tropic"),
                    os.path.join(meshcase_dir, "param.nml.tropic"),
                )
                shutil.copyfile(
                    os.path.join(modcase_dir, "param.nml.clinic"),
                    os.path.join(meshcase_dir, "param.nml.clinic"),
                )
                ...
                for cf in self.common_files:
                    ...
                    shutil.copyfile(
                        cff, os.path.join(meshcase_dir, os.path.basename(cff))
                    )

                # TROPIC ----------------------------------------------
                print(f"\t\t Handling the tropic inputs")

                bash_meshcase_tropic = os.path.join(
                    meshcase_dir,
                    os.path.basename(bash_case_tropic).replace("MESHNAME", meshname),
                )
                fmt_string_file(
                    self.bash_tropic,
                ...
                slurm_tropic_dict = {
                    ...
                    **{
                        ...
                        "baro": "tropic",
                        "output_log_file_base": self.output_log_file_base.format_map(
                            locals()
                        ),
                    },
                }

                for i, sct in enumerate(slurm_case_tropic):
                    smt = os.path.join(
                        meshcase_dir,
                        os.path.basename(sct).replace("MESHNAME", meshname),
                    )
                    if self.machine.lower() == "hpc5":
                        fmt_string_file(
                            self.slurm_tropic[i],
                ...
                mod_th_case = os.path.join(
                    meshcase_dir,
                    os.path.basename(self.mod_th_dict).replace("CASENAME", cname),
                )
                fmt_string_file(
                    self.mod_th_dict, mod_th_case, mod_th_files, method="replace"
                )
                ...
                bash_clinic_dict = {
                    ...
                    **{
                        ...
                        "{cname}": cname,
                        "{linked_th_file_strings}": linked_th_file_strings,
                        "{mesh_input_dir}": os.path.relpath(
                            mesh_input_dir, meshcase_dir
                        ),
                        "{case_year}": str(case_year),
                        "{meshname}": meshname,
                    },
                }
                bash_meshcase_clinic = os.path.join(
                    meshcase_dir,
                    os.path.basename(bash_case_clinic).replace("MESHNAME", meshname),
                )
                fmt_string_file(
                    self.bash_clinic,
                ...
                if self.machine.lower() == "hpc5":
                    ...
                    for i, scc in enumerate(slurm_case_clinic):
                        smc = os.path.join(
                            meshcase_dir,
                            os.path.basename(scc).replace("MESHNAME", meshname),
                        )
                        fmt_string_file(
                            self.slurm_clinic[i],
                            smc,
                    ...
                    az_yml_meshcase = os.path.join(
                        self.az_yml_dir,
                        os.path.basename(self.az_yml_file).replace(
                            "MESHNAME", meshname
                        ),
                    )
                    az_yml_meshcase = az_yml_meshcase.replace("CASENAME", cname)
                    fmt_string_file(
                        self.az_yml_file, az_yml_meshcase, az_dict, method="format_map"
                    )
                else:
                    raise ValueError(
    ...
    def set_schism_vars(self):
        ...
        self.param_tropic_base = self.inputs.get("param_tropic_base").format(
            **self.env_vars
        )
        self.param_clinic_base = self.inputs.get("param_clinic_base").format(
            **self.env_vars
        )
        ...
        self.slurm_tropic = [
            st.format(**self.env_vars) for st in self.inputs.get("slurm_tropic")
        ]
        self.slurm_clinic = [
            sc.format(**self.env_vars) for sc in self.inputs.get("slurm_clinic")
        ]
        ...
        self.common_files = [
            string.Formatter().vformat(cf, (), SafeDict((self.env_vars)))
            for cf in self.inputs.get("common_files")
        ]
        ...
        if "tide" in cp:
            ...
            if pdict["method"] == "shift":
                ...
                write_noaa(
                    self.noaa_pt_reyes,
                    pt_reyes,
                    os.path.join(modcase_dir, f"pt_reyes{tidal_pert}.csv"),
                )
                ...
                write_noaa(
                    self.noaa_monterey,
                    monterey,
                    os.path.join(modcase_dir, f"monterey{tidal_pert}.csv"),
                )
            else:
            ...
            if pdict["method"] == "set":
                ...
                dat_out = (
                    dat_in.copy() * 10
                )  # the input is just 0 -> 1 and it needs to be 0 ->10

            df["height"] = dat_out[1].to_list()

            # write out dataframe
            dated_fn = os.path.join(
                modcase_dir,
                os.path.basename(th_file).replace(".th", f".{cname}.dated.th"),
            )
            ...
            for gate in ["radial", "boat_lock", "flash"]:
                ...
                shutil.copyfile(
                    th_file_in, os.path.join(modcase_dir, os.path.basename(th_file_in))
                )
                clip_fn = os.path.join(
                    modcase_dir,
                    f'{os.path.basename(th_file_in).split("_")[0]}_{gate}.{cname}.th',
                )
                ...
                # add to list of files that are perturbed
                perturbed_th[
                    f'{os.path.basename(th_file_in).split("_")[0]}_{gate}.th'
                ] = clip_fn
            ...
            shutil.copyfile(
                th_file, os.path.join(modcase_dir, os.path.basename(th_file))
            )
            clip_fn = os.path.join(
...
if __name__ == "__main__":
    ...
    if True:  # run for HPC5 or Azure (set machine to hpc5 or azure)
