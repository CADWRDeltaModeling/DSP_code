...
colnames <- c('Time','radial','flash','boatlock')

mark1 <- read.csv(mark1_filename, col.names = colnames)
mark2 <- read.csv(mark2_filename, col.names = colnames)

mark1$Time <- lubridate::ymd(mark1$Time)
mark1$radial[mark1$radial==1] <- 0
mark1$radial[mark1$radial==-10] <- 1
mark2$Time <- lubridate::ymd(mark2$Time)
mark2$radial[mark2$radial==1] <- 0
mark2$radial[mark2$radial==-10] <- 1


# plot -------------------

suis.df <- merge(delta_state[,c('Time','Suisun Gate')], mark1[,c('Time','radial')], by='Time', all=TRUE)
suis.df <- merge(suis.df, mark2[,c('Time','radial')], by='Time', all=TRUE)
