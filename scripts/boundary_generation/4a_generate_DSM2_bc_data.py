...
def update_DSS(pdict, case_dir, crange, in_dss, out_dss, pathname, 
    ...
    margin_ts = df.copy()
    margin_ts = margin_ts[(margin_ts.index < ts_df.index[0]) | (margin_ts.index > ts_df.index[-1])]
    ...
    with pyhecdss.DSSFile(dss_out_file) as d_out:
        ...
        if e_part.startswith('IR-'):
            d_out.write_its(pathname_out, ts_df, unit_part, ptype) # write to output DSS file
        else:
            ts_df.index = ts_df.index + pd.Timedelta(days=1) # need to shift by one day because of DSS writing timestamp issues
            if ts_df.index.freq is None:
                ts_df.index.freq = pd.infer_freq(ts_df.index)
            d_out.write_rts(pathname_out, ts_df, unit_part, ptype) # write regular output to DSS file
