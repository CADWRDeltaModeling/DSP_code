...
infile = "./noaa_sffpx_9414290_water_level_1980_2026.csv"
outfile = infile.replace(".csv","_ft.csv")
...
with open(infile, "r") as inf, open(outfile, "w") as outf:
    for line in inf:
        ...
        if re.match(r"^\d{4}-\d{2}-\d{2}", line):  # Check if line starts with a date
            ...
            parts[1] = f"{m_to_ft(float(parts[1])):.2f}" if parts[1] != '' else parts[1] # Convert 2nd item to feet
