...
class ANNBCECGen(object):
    ...
    def get_meshcase_inouts(self, mesh, case_num):
        ...
        meshcase_out_dir = os.path.join(self.out_dir,f'{mesh}_{case_num}')
        if not os.path.exists(meshcase_out_dir):
            os.mkdir(meshcase_out_dir)
        # EC outputs
        ...
        ec_out_df = self.get_multiple_staout(outputs_fpath, time_basis, self.out_vars['ec_locs'])
        ...
        for invar in self.in_vars:
            ...
            elif invar['method'] == "read_single_flux": # outputs_fpath, time_basis, loc TODO: check inputs
                
                invar_df[in_name] = self.read_single_flux(invar['flux_output'].format_map({**self.env_vars,
                                                                                                **locals(),
                                                                                                **{"meshcase_dir": meshcase_dir},
                                                                                                **mesh_mod_th_dict}),
                                                            time_basis,
        ...
        tot_df = pd.merge(invar_df, ec_out_df, left_index=True, right_index=True, how='inner')
    ...
    def read_single_column_th(self, th_file, time_basis, th_header, in_col):
        ...
        out_df = out_df.resample('15T').ffill()

        return out_df
    
    def calc_dcu(self, th_files, time_basis):
        warnings.warn("'calc_dcu' method assumes an order to th_filse of 1) source 2) sink")
        ...
        net = src + sink
        net = net.resample('15T').ffill()

        return net
    
    def calc_tidal_energy(self, outputs_fpath, time_basis, loc):

        wse_ts = self.get_single_staout(outputs_fpath, time_basis, loc)
        ...
        df_tidal_energy = df_tidal_energy.resample('15T').ffill()

        return df_tidal_energy
    
    def get_single_staout(self, outputs_fpath, time_basis, loc, depth='upper'):

        all_ts = read_staout(outputs_fpath, self.station_fpath, time_basis)

        if isinstance(loc, list):
            loc = loc[0]

        if depth == 'upper':
            loc_outs = [stc for stc in self.station_df.index if loc in stc]
            if len(loc_outs) == 0:
    ...
    def get_multiple_staout(self, outputs_fpath, time_basis, locs):

        all_ts = read_staout(outputs_fpath, self.station_df, time_basis)
    ...
    def read_single_flux(self, outputs_fpath, time_basis, loc):
        ...
        flux_ts = flux_ts.resample('15T').ffill()
...
if __name__ == '__main__':
    ...
    annbc.get_meshcase_inouts('suisun', 4)