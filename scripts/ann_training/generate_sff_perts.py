...
sf_1hr_raw_ts = pd.read_csv(sf_1hr_fn, index_col=0, parse_dates=[0], 
                            usecols=["Date Time", "Water Level"], 
                            skipinitialspace=True,
                            dtype={"Water Level": str},
                            sep=',', comment='#')
sf_1hr_clean_inst = clean_to_date_range(sf_1hr_raw_ts)
sf_1hr_clean_inst["Water Level"] = pd.to_numeric(sf_1hr_clean_inst["Water Level"], errors="coerce")
...
sf_6min_raw_ts = pd.read_csv(sf_6min_fn, index_col=0, parse_dates=[0], 
                             usecols=["Date Time", "Water Level"], 
                             skipinitialspace=True,
                             dtype={"Water Level": str},
                             sep=',', comment='#')
sf_6min_clean_inst = clean_to_date_range(sf_6min_raw_ts, start=pd.to_datetime('2012-9-1'))
sf_6min_clean_inst["Water Level"] = pd.to_numeric(sf_6min_clean_inst["Water Level"], errors="coerce")
...
sf_harmonic_fn = "../boundary_generation/input/noaa_sffpx_9414290_prediction_1990_2025.csv"
sf_harm_raw_ts = pd.read_csv(sf_harmonic_fn, index_col=0, parse_dates=[0])
sf_harm_clean_inst = clean_to_date_range(sf_harm_raw_ts) # unecessary because it's all predicted anyway
...
reg_filt_df = pd.merge(mrz_15min_filt, pd.concat([sf_1hr_filt, sf_6min_filt]), how='left', left_index=True, right_index=True)
...
if False:
    ...
    plt.plot(mrz_15min_filt.index, mrz_15min_filt['mrz'], label='MRZ', linestyle='-', alpha=0.7)
    plt.plot(reg_filt_df.index, reg_filt_df['sf'], label='SF', linestyle='--', alpha=0.7)
    plt.plot(pred_filt_df.index, pred_filt_df['sf_pred'], label='SF Predicted', linestyle=':', alpha=0.7)
    plt.xlabel('')
...
pred_filt_df = pred_filt_df.loc[(sf_1hr_filt.index[-1] - pd.Timedelta(weeks=2)):(sf_6min_filt.index[0] + pd.Timedelta(weeks=2))]
...
if False:
    plt.figure(figsize=(12, 6))
    plt.plot(sf_1hr_filt.index, sf_1hr_filt['Water Level'], label='1 Hour Data', linestyle='-', alpha=0.7)
    plt.plot(sf_6min_filt.index, sf_6min_filt['Water Level'], label='6 Minute Data', linestyle='-.', alpha=0.7)
    plt.plot(pred_filt_df.index, pred_filt_df['Water Level'], label='SF Predicted', linestyle=':', alpha=0.7)
    plt.plot(sf_filt_df.index, sf_filt_df['Water Level'], label='SF Spliced', linestyle='--', alpha=0.7)
    plt.xlabel('')
...
reg_nrg_df = pd.merge(sf_harm_nrg, pd.concat([sf_1hr_nrg, sf_6min_nrg]), how='left', left_index=True, right_index=True)
...
if False:
    plt.figure(figsize=(12, 6))
    plt.plot(sf_harm_nrg.index, sf_harm_nrg['sf_harm'], label='SF Harmonic', linestyle=':', alpha=0.7)
    plt.plot(reg_nrg_df.index, reg_nrg_df['sf_obs'], label='SF Observed', linestyle=':', alpha=0.7)
    plt.plot(pred_nrg_df.index, pred_nrg_df['sf_pred'], label='SF Predicted', linestyle='--', alpha=0.7)
    plt.xlabel('')
...
pred_nrg_df = pred_nrg_df.loc[(sf_1hr_nrg.index[-1] - pd.Timedelta(weeks=2)):(sf_6min_nrg.index[0] + pd.Timedelta(weeks=2))]
...
if False:
    plt.figure(figsize=(12, 6))
    plt.plot(sf_1hr_nrg.index, sf_1hr_nrg['Water Level'], label='1 Hour Data', linestyle='-', alpha=0.7)
    plt.plot(sf_6min_nrg.index, sf_6min_nrg['Water Level'], label='6 Minute Data', linestyle='-.', alpha=0.7)
    plt.plot(pred_nrg_df.index, pred_nrg_df['Water Level'], label='SF Predicted', linestyle=':', alpha=0.7)
    plt.plot(sf_nrg_df.index, sf_nrg_df['Water Level'], label='SF Spliced', linestyle='--', alpha=0.7)
    plt.xlabel('')
    plt.ylabel('Tidal Energy  <(ft- <ft>)^2>')
    plt.legend()
    plt.grid(True)
...
# Regular
reg_out_df = pd.merge(sf_filt_df, sf_nrg_df, left_index=True, right_index=True, how='outer')
...
mrz_pert_df = pd.read_csv(pert_fn, index_col=[0], parse_dates=[0], header=None).resample('D', closed='right').mean()
mrz_filt_df = pd.read_csv(mrz_filt_fn, index_col=[0], parse_dates=[0]).resample('D', closed='right').mean()
...
subtide_pert_df = pd.merge(sf_subtide_pert, sf_nrg_df, left_index=True, right_index=True, how='outer')
subtide_pert_df.columns = ['sf_tidal_filter','sf_tidal_energy']

# Shifted Subtide + 7 days
[filt] = shift_ts([sf_filt_df], 7, 'shift+7')
shift_subtide_plus7_df = pd.merge(filt, sf_nrg_df, left_index=True, right_index=True, how='outer')
shift_subtide_plus7_df.columns = ['sf_tidal_filter','sf_tidal_energy']

# Shifted Subtide - 7 days
[filt] = shift_ts([sf_filt_df], -7, 'shift-7')
shift_subtide_minus7_df = pd.merge(filt, sf_nrg_df, left_index=True, right_index=True, how='outer')
...
subtide_pert_shift_plus7_df = pd.merge(filt, sf_nrg_df, left_index=True, right_index=True, how='outer')
subtide_pert_shift_plus7_df.columns = ['sf_tidal_filter','sf_tidal_energy']

# Subtidal Pert + Shifted Subtide - 7 days
[filt] = shift_ts([sf_subtide_pert], -7, 'shift-7')
subtide_pert_shift_minus7_df = pd.merge(filt, sf_nrg_df, left_index=True, right_index=True, how='outer')
