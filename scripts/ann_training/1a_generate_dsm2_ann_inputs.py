...
def generate_ann_inputs(
    ...
    b_part = "RSAC128"
    c_part = "POS"
    dcc_op_df = process_gate_data(
        gate_dss_file, b_part, c_part, map_zero_one=["df<2", "df==2"]
    )
    ...
    df_nrg = cosine_lanczos(
        (df_mtz_stage - df_filter) ** 2, cutoff_period="40H", padtype="odd"
    )  # = < (z- <z>)^2 >
    ...
    out_df = pd.merge(
        out_df, df_model_ec, left_index=True, right_index=True, how="outer"
    )
