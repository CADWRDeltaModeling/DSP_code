...
start = dtm.datetime(2015, 2, 18)
version = "suisun-suisun"
outdir = f"/scratch/tomkovic/DSP_code/model/schism/roundtrip/{version}"
mod_home = f"/scratch/tomkovic/DSP_code/model/schism/roundtrip/{version}"
...
def custom_name_transform(input_name):
    ...
    transformed_name = base_name.replace("repo", "output").replace(
        ".th", "_elapsed_20150218.th"
    )
    return transformed_name


th_repo = os.path.join(mod_home, f"*{version}.dated.th")
